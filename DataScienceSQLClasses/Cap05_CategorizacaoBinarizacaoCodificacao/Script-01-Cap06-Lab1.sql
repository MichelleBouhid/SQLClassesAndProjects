...
-- Binarização da variável classe (0/1) - Faz um de - para
SELECT 
  CASE 
...
-- OBS: Estamos modificando em memória, o select nao slava a tabela
...
-- Mas dessa maneira não pode usar pra machine learning
SELECT 
  CASE 
...
-- Melhor ordenar pra ficar mais organizado pra raciocinar
...
-- Aqui só aplicamos categorização, não convertemos a variavel para uma representação numerica
-- Ess maneira é indicada para por ex, prepaar dados para apresnetação no Power BI
SELECT 
  CASE 
...
-- Aqui convertemos a variavel para representação numerica, então Label Enconding
-- Aqui preparamos os dados para Machine Learning
SELECT 
  CASE 
...
-- Importante colocar o ELSE, porque pega algum possivel erro de digitação
SELECT 
  CASE 
